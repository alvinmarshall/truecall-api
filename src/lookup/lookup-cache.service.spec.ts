...
describe('LookupCacheService', () => {
  ...
  describe('get', () => {
    ...
    it('returns null when entry is expired', async () => {
      const { service } = makeService({
        findOne: jest
          .fn()
          .mockResolvedValue({ result: mockResult, expiresAt: pastDate() }),
      });
      expect(await service.get('+233201234567')).toBeNull();
    });
    ...
    it('returns cached result when entry is valid', async () => {
      const { service } = makeService({
        findOne: jest
          .fn()
          .mockResolvedValue({ result: mockResult, expiresAt: futureDate() }),
      });
  ...
  describe('set', () => {
    ...
    it('calculates expiresAt from CACHE_TTL_SECONDS', async () => {
      ...
      const { expiresAt } = (
        repo.upsert.mock.calls[0] as [{ expiresAt: Date }, string[]]
      )[0];
